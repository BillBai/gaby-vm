...
enum class RegisterStatus : int {
  // The range was predecoded; its instructions now dispatch through the cache.
  Ok = 0,
  // `size_bytes` was zero or not a multiple of the 4-byte instruction size.
  InvalidArgument,
  // The range overlaps an already-registered range — any overlap, including an
  // exact duplicate. The embedder owns de-duplication.
  OverlappingRange,
  // Some instruction in the range needs a CPU feature the cache's
  // CPUFeaturesAuditor does not accept. GetLastErrorDetail() locates it.
  UnsupportedFeature,
...
//
// Plain-old-data by design: it holds no std::string and owns nothing. `reason`
// and `missing_features` point at storage owned by the cache and valid only
// until the next RegisterCodeRange call on the same cache. Querying it is not
// thread-safe with respect to a concurrent RegisterCodeRange — the caller must
// query on the same thread that made the failed call. Mirrors the errno /
// strerror split (see design doc 4.3.2): the status code stays simple, the
// multi-dimensional detail is fetched separately.
struct ErrorDetail {
  // The failure this detail describes. `Ok` means no failure has been recorded.
  RegisterStatus status;
  // Host address of the offending instruction. Valid for UnsupportedFeature;
  // 0 otherwise.
  uintptr_t pc;
...
//
// Exactly 16 bytes so a registered code range maps to a flat entries[] array
// indexed by (pc - range_start) / 4, making the cache-track lookup a single
// array load. `leaf` is an opaque handle to the resolved VIXL leaf dispatcher;
// only predecode_cache.cc (and the cache-track execution path) interprets it,
// which keeps this public header free of any vixl:: type. The V1 entry stores
// the (form_hash, leaf) pair directly — the per-form thunk of the design doc
// 4.2.1 is deferred to V2. See design.md D8.
struct PredecodedEntry {
  // VIXL form hash for this instruction. The cache-track execution path writes
  // it to Simulator::form_hash_ before invoking the leaf, because shared
  // Simulate_* leaves branch on form_hash_ (deep-dive R1).
  uint32_t form_hash;
...
// RegisterCodeRange and the cache builds the record. The type is public only
// so the cache-track execution path can perform its hot-path lookup inline:
// given a CodeRange and a PC inside it, the entry is &entries[(pc - start)/4].
//
...
class PredecodeCache {
 public:
  ...
  // result, GetLastErrorDetail() returns the corresponding detail.
  ...
  // Never null; `status` is Ok if no failure has been recorded. The pointee is
  // valid until the next RegisterCodeRange call (see ErrorDetail).
  const ErrorDetail* GetLastErrorDetail() const;

  // Find the registered range containing host address `pc`, or null if `pc`
  // lies in no registered range. This is the cache-track slow-path lookup: it
  // takes the range table's shared lock and runs a binary search. The
  // per-instruction fast path (a hit in the Simulator's cached range) does not
  // call this. Not part of the embedder-facing surface — it is the integration
  // seam used by the cache-track execution path.
