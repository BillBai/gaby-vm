...
VIXL source file, or any public header under `include/gaby_vm/`. The
harness consumes only the existing `src/` headers already consumed by
`test/` and introduces no new public API surface.
