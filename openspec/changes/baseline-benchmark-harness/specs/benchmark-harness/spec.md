...
### Requirement: Change does not modify src/, include/gaby_vm/, or imported VIXL files

This change SHALL NOT alter any file under `src/`, any imported VIXL
source file, or any public header under `include/gaby_vm/`. The harness
consumes only the existing `src/` headers already consumed by `test/` and
introduces no new public API surface.

#### Scenario: Diff against merge base is confined to bench/ and top-level CMakeLists.txt

- **WHEN** the diff for this change is computed against its merge base on `main`
- **THEN** every modified or added file path is one of: `CMakeLists.txt` at the repo root, a file under `bench/`, or a file under `openspec/changes/baseline-benchmark-harness/`

#### Scenario: No new public headers are introduced

- **WHEN** the contents of `include/gaby_vm/` (if present) are listed before and after this change
- **THEN** the two listings are identical (no files added, removed, or modified)
