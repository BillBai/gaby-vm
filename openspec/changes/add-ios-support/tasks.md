...
- [ ] 2.2 Extract each correctness suite body into a callable entry; reduce its CTest executable to a thin `main()` over it. Verify `ctest --test-dir build/debug -R vixl_port` and the full CTest suite stay green. (Done for vixl_port: integer/fp/neon each expose `run_vixl_port_<family>()` in the same TU as their registrations; host CTest stays green, 9/9 vixl_port+vixl_asm. Plus an iOS-only `run_vixl_port_all()` that walks the combined registered set once under a summed baseline — needed because all three families link into one XCTest process, unlike the per-family CTest exes. Remaining: the baseline/unit suites.)
...
- [ ] 3.3 Add XCTest cases that call the correctness-suite entries (one suite → pass/fail). Verify `xcodebuild test` on an arm64 Simulator runs all suites green. (Done for vixl_port: `VixlPortTests.testVixlPortAllFamilies` runs the combined integer+fp+neon set green on the Simulator — 520 passed / 69 skipped / 0 FAILED, matching the summed macOS baseline. Remaining: the baseline/unit suites.)
