...
- [x] 10.4 Archive the change: `openspec archive baseline-correctness-test` (folds the spec delta into `openspec/specs/aarch64-simulator/spec.md` and moves the change folder to `openspec/changes/archive/`). — Executed via `openspec archive baseline-correctness-test --yes`. Output: "Specs to update: aarch64-simulator: update / Applying changes to openspec/specs/aarch64-simulator/spec.md: + 2 added / Totals: + 2, ~ 0, - 0, → 0 / Specs updated successfully. / Change 'baseline-correctness-test' archived as '2026-05-12-baseline-correctness-test'." Two new requirements folded into the canonical `aarch64-simulator` spec; change directory now at `openspec/changes/archive/2026-05-12-baseline-correctness-test/`.
